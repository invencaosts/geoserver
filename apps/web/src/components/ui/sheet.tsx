...
function SheetHeader({ className, eyebrow, ...props }: React.ComponentProps<"div"> & { eyebrow?: string }) {
  return (
    <div
    ...
    >
      {eyebrow && (
        <span className="text-[10.5px] font-semibold uppercase tracking-wider text-primary">{eyebrow}</span>
      )}
      {props.children}
    </div>
  )
